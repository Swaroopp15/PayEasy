...
transactionRouter.post("/init", async (req, res) => {
  ...
  try {
    ...
    return res
      .status(400)
      .json({
        message: `Error occurred at transaction initializing: ${error}`,
      });
  }
});
...
transactionRouter.post("/credit", async (req, res) => {
  ...
  try {
    ...
    const recipient = await userModel.findOne({
      email: transaction.reciptentEmail,
    });
    ...
    res
      .status(400)
      .json({
        message: "Error occurred while crediting recipient",
        error: error.message,
      });
  }
});

transactionRouter.post("/cancel", async (req, res) => {
  const { transaction_id } = req.body;
  ...
  try {
    ...
    // Get user ID and refund the balance
    ...
    console.log(`Refunded ${transaction.amount} to user ${userId}`);

    res.status(200).json({ message: "Transaction cancelled successfully and amount refunded." });

  } catch (error) {
    console.error("Error at cancelling transaction:", error);
    res.status(500).json({
      message: "Error occurred while cancelling transaction",
      error: error.message,
    });
  }
});
